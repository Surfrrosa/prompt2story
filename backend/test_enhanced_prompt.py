...
sys.path.append('app')
from main import load_prompt  # noqa: E402

load_dotenv()
client = openai.OpenAI(api_key=os.getenv('OPENAI_API_KEY'))

test_input = (
    "So, we've had a couple of users report that when they submit the form, "
    "it just hangs. Might be some kind of edge case tied to network retries "
    "or error messages not rendering. We'll need to double-check form "
    "validation too — especially mobile. Also, we forgot to account for new "
    "roles added to the admin panel last sprint — permissions aren't being "
    "respected properly.\n\n"
    "John said localization isn't showing for French, but only in the "
    "pricing table and CTA button. Probably missing some keys.\n\n"
    "Also let's not forget: the modal should auto-dismiss after success, "
    "which it currently doesn't — user gets stuck. And QA noticed we're not "
    "logging failed submissions for diagnostics. That's all I have unless "
...
try:
    response = client.chat.completions.create(
        ...
        messages=[
            {'role': 'system',
             'content': ('You are an expert product manager and '
                         'business analyst.')},
    ...
    if start_idx != -1 and end_idx != 0:
        ...
        if len(result['user_stories']) >= 6:
            ...
            print(f'❌ ISSUE: Only generated {story_count} stories, '
                  f'expected 6-7')
